...
impl ByteWriter for V4Data {
    fn write_pkt(&self, buf: &mut Vec<u8>) -> std::io::Result<()> {
        if self.active_newgrf.len() > 255 {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "NewGRF maximum number is 255"));
        }

        buf.push(self.active_newgrf.len() as u8);
        for (id, hash) in self.active_newgrf.clone().into_iter().collect::<BTreeMap<_, _>>().into_iter() {
...
mod tests {
    ...
    pub(crate) fn fixtures() -> (Vec<u8>, ServerResponse) {
        let b = vec![
            0x04, 0x03, 0x44, 0x4E, 0x07, 0x00, 0x48, 0xB3, 0xF9, 0xE4, 0xFD, 0x0D, 0xF2, 0xA7, 0x2B, 0x5F, 0x44, 0xD3, 0xC8, 0xA2, 0xF4,
            0xA0, 0x4D, 0x47, 0x03, 0x05, 0x2E, 0x96, 0xB9, 0xAB, 0x2B, 0xEA, 0x68, 0x6B, 0xFF, 0x94, 0x96, 0x1A, 0xD4, 0x33, 0xA7, 0x01,
            0x32, 0x32, 0x33, 0x22, 0x31, 0x61, 0x80, 0xDA, 0x1B, 0xA6, 0x44, 0x4A, 0x06, 0xCD, 0x17, 0xF8, 0xFA, 0x79, 0xD6, 0x0A, 0x63,
            0xEC, 0x0A, 0x00, 0x63, 0xEC, 0x0A, 0x00, 0x0F, 0x00, 0x0A, 0x4F, 0x6E, 0x6C, 0x79, 0x46, 0x72, 0x69, 0x65, 0x6E, 0x64, 0x73,
            0x20, 0x4F, 0x70, 0x65, 0x6E, 0x54, 0x54, 0x44, 0x20, 0x53, 0x65, 0x72, 0x76, 0x65, 0x72, 0x20, 0x23, 0x31, 0x00, 0x31, 0x2E,
            0x35, 0x2E, 0x33, 0x00, 0x16, 0x00, 0x19, 0x00, 0x00, 0x52, 0x61, 0x6E, 0x64, 0x6F, 0x6D, 0x20, 0x4D, 0x61, 0x70, 0x00, 0x00,
